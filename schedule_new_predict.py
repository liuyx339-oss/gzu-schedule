...
TARGET_HOURS.update({p: 60.0 for p in STAFF["rad_docs_pt"]})
TARGET_HOURS.update({p: 80.0 for p in STAFF["rad_techs_pt"]})
TARGET_HOURS["Li anhua"] = 32.0

# ==========================================
# 🌟 2. 注入基于 AI 潮汐算出的【全新优化班表】(Optimized Shifts)
# ==========================================
SHIFT_DICT = {
    "Opt_D1": (
        9.0,
        ...
        8.5,
        "白班",
    ),  # 核心主力班：09:00-18:00 (完美覆盖高峰，不留傍晚断档)
    "Opt_D2": (8.5, 17.5, 8.5, "白班"),  # 错峰早班：08:30-17:30 (处理极早期病人)
    "Opt_D3": (10.0, 19.0, 8.5, "白班"),  # 晚早班：10:00-19:00 (压制傍晚尾单，掩护夜班)
    "Opt_H1": (9.0, 13.0, 4.0, "半天班"),  # 精准早峰班：09:00-13:00
    "Opt_H2": (14.0, 18.0, 4.0, "半天班"),  # 精准晚峰班：14:00-18:00
    # 夜间基础班次保持需求文档的硬性规定不变
    "N": (17.5, 32.0, 14.5, "夜班"),
    "兼职夜": (18.0, 32.0, 14.0, "夜班"),
    "N(OnCall)": (17.5, 32.0, 14.5, "夜班"),
    "H3(专家)": (13.5, 17.5, 4.0, "半天班"),  # 李主任专属
}
...
DAY_SHIFTS_POOL = ["Opt_D1", "Opt_D2", "Opt_D3"]
HALF_SHIFTS_POOL = ["Opt_H1", "Opt_H2"]
...
def run_smart_roster():
    ...
    output_file = os.path.join(base_dir, "Shift_Matrix_With_Names_Predict.csv")
    df_out.to_csv(output_file, index=False, encoding="utf-8-sig")
    print("✅ 包含【优化新班表】的完美排班矩阵已生成！请转JSON并刷新大屏。")
